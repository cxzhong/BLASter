...
class LLLResult:
    ...
    def __init__(self, reduced_basis: np.ndarray, transformation: np.ndarray, 
                 time_profile: TimeProfile, original_basis: np.ndarray):
    ...
    def __repr__(self) -> str:
        return (f"LLLResult(dimension={self.reduced_basis.shape}, "
                f"rhf={self.rhf:.6f}, slope={self.slope:.6f}, "
                f"iterations={self.time_profile.num_iterations})")


def lll_reduce(basis, delta=0.99, block_size=None, cores=None, 
               use_seysen=False, verbose=True, **kwargs):
    """
...
def bkz_reduce(basis: np.ndarray,
               beta: int,
               tours: int = 1,
               **kwargs) -> LLLResult:
    """
...
def estimate_reduction_quality(basis: np.ndarray, row_vectors: bool = True) -> Dict[str, float]:
    """
